...
namespace Naninovel.Commands
{
    ...
    /// <br/><br/>
    /// In case variable name starts with `T_` or `t_` it's considered a reference to a value stored in 'Script' [managed text](/guide/managed-text) document. 
    ...
    public class SetCustomVariable : Command, Command.IForceWait
    {
        ...
        /// variable to assign and `ExpressionBody` is a [script expression](/guide/script-expressions), the result of which should be assigned to the variable.
        /// <br/><br/>
        ...
        {
            Err($"Failed to evaluate set expression `{Expression}`. {desc ?? string.Empty}");
        }
    }
}
