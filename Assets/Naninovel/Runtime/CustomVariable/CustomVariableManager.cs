...
namespace Naninovel
{
    ...
    public class CustomVariableManager : IStatefulService<GameStateMap>, IStatefulService<GlobalStateMap>, ICustomVariableManager
    {
        ...
        public virtual bool VariableExists (string name)
        {
            ...
            return CustomVariablesConfiguration.IsGlobalVariable(name)
                ? globalVariableMap.ContainsKey(name)
                : localVariableMap.ContainsKey(name);
        }
        ...
        public virtual void ResetLocalVariables ()
        {
            ...
            foreach (var varData in Configuration.PredefinedVariables)
            {
                ...
                var value = ExpressionEvaluator.Evaluate<string>(varData.Value,
                    e => LogInitializeVarError(varData.Name, varData.Value, e));
        ...
        private void LogInitializeVarError (string varName, string expr, string error) =>
            Engine.Warn($"Failed to initialize `{varName}` variable with `{expr}` expression: {error}");
    }
}
