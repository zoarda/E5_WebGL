...
namespace Naninovel
{
    ...
    public class ScriptAssetSerializer
    {
        ...
        {
            AppendPrefix();
            var shouldSkipFirst = line.InlinedCommands.FirstOrDefault() is ModifyCharacter c && c.IsGenericPrefix;
            for (int i = shouldSkipFirst ? 1 : 0; i < line.InlinedCommands.Count; i++)
                AppendInlined(line.InlinedCommands[i]);

            void AppendPrefix ()
            {
                if (line.InlinedCommands.FirstOrDefault() is ModifyCharacter mod && mod.IsGenericPrefix)
